...
%%private(
  let query1IR: IR.t = %raw(`{"usedParamSet":{"premise_id":true},"params":[{"name":"premise_id","required":true,"transform":{"type":"scalar"},"locs":[{"a":33,"b":44}]}],"statement":"SELECT * FROM premise WHERE id = :premise_id!"}`)
)
