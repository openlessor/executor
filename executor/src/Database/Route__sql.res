...
@gentype
type query1Params = {route_root: string}
...
%%private(
  let query1IR: IR.t = %raw(`{"usedParamSet":{"route_root":true},"params":[{"name":"route_root","required":true,"transform":{"type":"scalar"},"locs":[{"a":150,"b":161}]}],"statement":"SELECT id, name, description, updated_at, route_root FROM premise_route LEFT JOIN premise ON premise.id = premise_route.premise_id WHERE route_root = :route_root!"}`)
)
