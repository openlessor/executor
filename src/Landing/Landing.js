// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Card from "../Ui/Card.js";
import * as Cart from "../Cart/Cart.js";
import * as Icon from "../Ui/Icon.js";
import * as State from "../State/State.js";
import * as React from "react";
import * as Js_array from "@rescript/runtime/lib/es6/Js_array.js";
import * as Container from "../Ui/Container.js";
import * as Belt_Array from "@rescript/runtime/lib/es6/Belt_Array.js";
import * as DatePicker from "../DatePicker/DatePicker.js";
import * as TiliaReact from "@tilia/react/src/TiliaReact.js";
import * as InventoryList from "../InventoryList/InventoryList.js";
import * as JsxRuntime from "react/jsx-runtime";
import * as ReservationTypeSelection from "../ReservationTypeSelection/ReservationTypeSelection.js";

function str(prim) {
  return prim;
}

function addToCart(state, id) {
  return {
    cart: Belt_Array.concat(state.cart, [id]),
    selected_item: state.selected_item,
    items: state.items
  };
}

function removeFromCart(state, id) {
  return {
    cart: Js_array.filter(compareId => compareId !== id, state.cart),
    selected_item: state.selected_item,
    items: state.items
  };
}

let make = TiliaReact.leaf(props => {
  let unit = State.main_store.unit;
  let now = new Date();
  let today = new Date(now.setHours(0.0, 0.0, 0.0, 0.0));
  let match = React.useState(() => today);
  let setOpenDate = match[1];
  let openDate = match[0];
  let match$1 = React.useState(() => today);
  let setCloseDate = match$1[1];
  let closeDate = match$1[0];
  React.useEffect(() => {
    console.log("Open Date:");
    console.log(openDate);
    console.log("Close Date:");
    console.log(closeDate);
    return () => {};
  }, [
    openDate,
    closeDate
  ]);
  let updateOpenDate = openDate => setOpenDate(_prev => {
    if (openDate == null) {
      return today;
    } else {
      return openDate;
    }
  });
  let updateCloseDate = closeDate => setCloseDate(_prev => {
    if (closeDate == null) {
      return today;
    } else {
      return closeDate;
    }
  });
  let match$2 = React.useReducer((state, action) => {
    console.log("calling reducer");
    console.log({
      state: state,
      action: action
    });
    let result;
    result = action.TAG === "AddToCart" ? addToCart(state, action.id) : removeFromCart(state, action.id);
    console.log({
      nextState: result
    });
    return result;
  }, {
    cart: [],
    selected_item: undefined,
    items: []
  });
  let state = match$2[0];
  let cartCount = state.cart.length;
  return JsxRuntime.jsxs(Container.make, {
    children: [
      JsxRuntime.jsx(Card.make, {
        children: JsxRuntime.jsx("h1", {
          children: JsxRuntime.jsxs("span", {
            children: [
              JsxRuntime.jsx(Icon.MonitorCloud.make, {
                size: 48,
                className: "text-slate-400 mr-2 my-auto inline content-start"
              }),
              "Cloud Hardware Rental"
            ]
          }),
          className: "text-xl"
        }),
        className: "bg-slate-200/40 border-slate-200/40 border-1"
      }),
      JsxRuntime.jsxs(Card.make, {
        children: [
          JsxRuntime.jsxs("span", {
            children: [
              JsxRuntime.jsx(Icon.Clock.make, {
                size: 48,
                className: "text-slate-400 mr-2 my-auto inline content-start"
              }),
              "Select your reservation type: "
            ],
            className: "align-middle text-lg"
          }),
          JsxRuntime.jsx(ReservationTypeSelection.make, {}),
          JsxRuntime.jsxs("div", {
            children: [
              JsxRuntime.jsx(Icon.Calendar.make, {
                size: 48,
                className: "absolute left-0 top-0 bottom-0 my-auto text-slate-400"
              }),
              JsxRuntime.jsx("span", {
                children: "Select your reservation start date: ",
                className: "align-middle text-lg pl-[56px]"
              }),
              JsxRuntime.jsx(DatePicker.make, {
                calendarClassName: "bg-white",
                className: "block outline-slate-400 outline-1 px-2",
                isOpen: false,
                minDate: today,
                onChange: updateOpenDate,
                selected: openDate
              }),
              JsxRuntime.jsx("span", {
                children: "Select your reservation end date: ",
                className: `align-middle text-lg pl-[56px] ` + (
                  unit === "hour" ? "hidden" : ""
                )
              }),
              JsxRuntime.jsx(DatePicker.make, {
                calendarClassName: "bg-white",
                className: `block align-end outline-slate-400 outline-1 px-2 ` + (
                  unit === "hour" ? "hidden" : ""
                ),
                isOpen: false,
                minDate: openDate,
                onChange: updateCloseDate,
                selected: closeDate
              })
            ],
            className: "col-span-full grid grid-cols-subgrid relative"
          })
        ],
        className: "grid grid-cols-[auto_1fr] bg-white/20 gap-4 place-items-start items-center"
      }),
      JsxRuntime.jsx(Cart.StateContext.Provider.make, {
        children: JsxRuntime.jsxs(Cart.DispatchContext.Provider.make, {
          children: [
            JsxRuntime.jsx(InventoryList.make, {
              openDate: openDate,
              closeDate: closeDate
            }),
            JsxRuntime.jsx(Cart.make, {
              count: cartCount
            })
          ],
          value: match$2[1]
        }),
        value: state
      }),
      JsxRuntime.jsx("div", {
        children: JsxRuntime.jsx("button", {
          children: "Book Reservation",
          className: "mx-auto mt-4 bg-slate-500 hover:bg-slate-700 text-white py-2 px-4 rounded-sm"
        }),
        className: "w-full"
      })
    ]
  });
});

export {
  str,
  addToCart,
  removeFromCart,
  make,
}
/* make Not a pure module */
