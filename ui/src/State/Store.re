...
let makeServerStore =
  ...
  | Some(_) =>
    JsError.throwWithMessage(
      "This function should never run in the client context",
    )
  | None =>
    let store = makeStore(initialExecutorConfig);
    getAsyncLocalStorage()
    ->RescriptBun.AsyncHooks.AsyncLocalStorage.run(store, callback);
  };
};

let getServerStore = () => {
  getAsyncLocalStorage()
  ->RescriptBun.AsyncHooks.AsyncLocalStorage.getStore
