// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Pricing from "../Pricing/Pricing.mjs";
import * as TiliaReact from "@tilia/react/lib/es6/src/TiliaReact.mjs";
import * as JsxRuntime from "react/jsx-runtime";
import * as RescriptReactRouter from "@rescript/react/lib/es6/src/RescriptReactRouter.mjs";

function str(prim) {
  return prim;
}

let make = TiliaReact.leaf(props => {
  let item = props.item;
  let id = item.id;
  let image = "https://random.danielpetrica.com/api/random?" + String(id);
  let matches = [];
  let matchCount = matches.length;
  if (matchCount > 0) {
    String(matches.length);
  }
  String(item.quantity - matchCount | 0);
  return JsxRuntime.jsx("a", {
    children: JsxRuntime.jsxs("button", {
      children: [
        JsxRuntime.jsx("div", {
          children: JsxRuntime.jsx("img", {
            className: "p-[1.5] w-40 h-40",
            style: {
              width: "100%"
            },
            src: image
          }),
          className: "rounded-sm shadow-sm m-0 p-0"
        }),
        JsxRuntime.jsx("div", {
          children: JsxRuntime.jsx("h2", {
            children: item.name,
            className: "tracking-wider text-xs px-2"
          }),
          className: "flex flex-row justify-between w-full bg-gray-300 text-white shadow-sm"
        }),
        JsxRuntime.jsxs("div", {
          children: [
            JsxRuntime.jsx("p", {
              children: item.description,
              className: "text-xs text-left m-2"
            }),
            JsxRuntime.jsx(Pricing.make, {
              period_list: item.period_list
            })
          ],
          className: "flex flex-col flex-grow flex-1 w-full bg-white/40 rounded-sm m-[1.5] justify-between items-end"
        })
      ],
      className: "relative m-[1.5] flex flex-1 flex-col flex-grow max-w-40"
    }),
    className: "flex flex-1 flex-col flex-grow border-2",
    id: "item-" + String(id),
    href: "/item/" + String(id),
    onClick: e => {
      e.preventDefault();
      RescriptReactRouter.replace("/item/" + String(id));
    }
  });
});

export {
  str,
  make,
}
/* make Not a pure module */
