...
[@mel.send] external run: (t('store), 'store, unit => 'ret) => 'ret = "run";
...
let getStoreExn = (als: t('store)) => als->getStore->Option.get;

/* AsyncLocalStorage.exit(callback) */
[@mel.send] external exit: (t('store), unit => 'ret) => 'ret = "exit";

/* AsyncLocalStorage.enterWith(store) */
[@mel.send] external enterWith: (t('store), 'store) => unit = "enterWith";
